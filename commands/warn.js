...
const configPath = path.join(__dirname, '../config.json'); const MAX_WARNS = 4;
...
async execute({ sock, message, jid }) {
    ...
    if (!botIsAdmin) {
        return sock.sendMessage(jid, {
            text:key?.participant || message?.key?.remoteJid;
    const senderData = participants.find(p => p.id === se        });
    }
